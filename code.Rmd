...
default_steps <- 4              # The number of states one descends after a claim
default_prob  <- 0.05           # Average claim frequency
default_gamma <- 4              # Parameter used for the gamma distribution
...
```{r}
steady_state_premiums <- premiums()
loimaranta_efficiency <- function(lambda, premiums = steady_state_premiums, infinitesimal = default_infinitesimal) {
  steady_state_premium <- sum(steady_states(lambda) * premiums)
  steady_state_premium_new <- sum(steady_states(lambda + infinitesimal) * premiums)
  
  result <- (log(steady_state_premium_new) - log(steady_state_premium)) / (log(lambda + infinitesimal) - log(lambda))
  return(result)
}
```
...
```{r}
loimaranta_efficiency(lambda, steady_state_premiums)
```
...
dutch_loimaranta_efficiency <- function(lambda, infinitesimal = default_infinitesimal) {
  steady_state_premium <- dutch_steady_state_premium(lambda)
